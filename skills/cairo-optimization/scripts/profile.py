...
def _profile_filename(output_dir: str, package: str, name: str, metric: str, commit: str, ext: str) -> str:
    """Generate profile filename with timestamp and git hash."""
...
def _find_package_dir(repo_root: str, package: str) -> str:
    ...
    # Root package is valid only when explicitly requested or when names match.
    root_manifest = os.path.join(repo_root, "Scarb.toml")
    if os.path.isfile(root_manifest):
        if package in ("", ".", None):
            return repo_root
        try:
            manifest_data = tomllib.loads(Path(root_manifest).read_text(encoding="utf-8"))
        except Exception:
            manifest_data = {}
        root_package_name = manifest_data.get("package", {}).get("name")
        if isinstance(root_package_name, str) and root_package_name == package:
            return repo_root
